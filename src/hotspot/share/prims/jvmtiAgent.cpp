...
static OnLoadEntry_t lookup_On_Load_entry_point(JvmtiAgent* agent,
  ...
  if (!agent->is_loaded()) {
    if (!load_agent_from_executable(agent, on_load_symbols, num_symbol_entries)) {
      void* const library = load_library(agent, on_load_symbols,
                                         num_symbol_entries,
                                         /* vm exit on error */ !ignore_errors);
...
static OnLoadEntry_t lookup_JVM_OnLoad_entry_point(JvmtiAgent* lib,
  ...
  return lookup_On_Load_entry_point(lib, on_load_symbols,
                                    sizeof(on_load_symbols) / sizeof(char*),
                                    ignore_errors);
}
...
static OnLoadEntry_t lookup_Agent_OnLoad_entry_point(JvmtiAgent* agent,
  ...
  return lookup_On_Load_entry_point(agent, on_load_symbols,
                                    sizeof(on_load_symbols) / sizeof(char*),
                                    ignore_errors);
}
